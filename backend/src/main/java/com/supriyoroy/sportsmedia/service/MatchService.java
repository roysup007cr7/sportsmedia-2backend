...
public class MatchService {
    ...
    /** Helper to update fields on an entity from MatchRequest DTO */
    public void updateEntityFromRequest(MatchEntity m, MatchRequest req) {
        m.setHomeTeam(req.homeTeam());
        m.setAwayTeam(req.awayTeam());
        m.setHomeLogo(req.homeLogo());
        m.setAwayLogo(req.awayLogo());
        m.setDescription(req.description());
        m.setKickoffUtc(req.kickoffUtc());
        if (req.status() != null) {
            m.setStatus(MatchStatus.valueOf(req.status()));
        }
        m.setHomeScore(req.homeScore());
        m.setAwayScore(req.awayScore());
        m.setStreamUrl(req.streamUrl());
        m.setExtStream1(req.extStream1());
        m.setExtStream2(req.extStream2());
        m.setExtStream3(req.extStream3());
        m.setBackupStreams(req.backupStreams());
        m.setPosterUrl(req.posterUrl());
        m.setFeatured(req.featured() != null && req.featured());
        m.setVisible(req.visible() == null || req.visible());
    }
